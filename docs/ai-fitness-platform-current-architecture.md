...
> 2026-09-05 核对补充：后续开发以 [版本治理优化计划](agent-rag-versioning-optimization-plan.md) 为准。
> 下文企业增强示例为设计目标。force=True 仅跳过去重，不绕过同版本 checksum 冲突或向量维度约束；
> 当前重建是单文档原地替换，整库换模型需补隔离构建和原子切换。2026-09-05 已补文档主表的机构/所有者来源作用域、发布属性指纹和上传幂等约束；真实 PostgreSQL 并发验收仍按专项计划执行。

当前实现中有三类需要区分的哈希：

1. **原始文件哈希**：上传时对文件字节计算 SHA-256，保存于
   `knowledge_ingestion_jobs.content_sha256`。它用于绑定审核报告、验证暂存对象没有被篡改，
   不单独决定是否跳过 Embedding。

   例：管理员上传 `warmup.pdf` 后得到哈希 `abc123...`；审批时报告绑定这个哈希，Worker 执行前
   再读取文件计算一次。如果前后哈希不同，任务失败，不允许把被替换的文件发布到知识库。

2. **规范化文档 checksum**：文件完成解析和清洗后，对实际准备进入索引的内容计算 SHA-256，保存于
   `knowledge_documents.checksum`。它用于判断同一个 `source_uri` 的已发布内容是否发生变化。

   例：同一份 Markdown 只把连续空格改成一个空格，清洗后的内容不变，checksum 不变，因此再次上传时
   返回 `SKIPPED_UNCHANGED`，不会再次调用 Embedding，也不会新增检索切片。

3. **chunk 内容哈希**：每个切片对自身正文计算 SHA-256，保存于
   `knowledge_chunks.content_hash`，用于切片审计和稳定 ID。

   例：一篇文档第 3 个切片的正文从“热身 5 分钟”改成“热身 8 分钟”，该切片的
   `content_hash` 会变化，便于定位具体发生变化的内容；切片 ID 也会按新的内容重新生成。
...
| 场景 | 当前处理 | 示例 |
| --- | --- | --- |
| 同一来源、同一内容 | 跳过索引 | `knowledge://basic-training` 重复上传同一文件，复用当前已发布版本 |
| 文件名变化但来源和内容相同 | 仍然跳过 | `warmup-v1.pdf` 改名为 `warmup-final.pdf`，但 `source_uri` 和规范化内容不变 |
| 同一来源、内容变化 | 创建新文档版本 | `knowledge://basic-training` 从版本 1 更新到版本 2，旧版本归档，新版本发布 |
| 内容相同但 source_uri 不同 | 当前不会全库去重 | 同一 PDF 分别上传到 `knowledge://org-a/a` 和 `knowledge://org-a/b`，会被当作两个来源 |
| 内容相同但权限范围不同 | 不应强行合并 | 全平台指南和某机构内部指南即使正文相同，也要分别保留权限边界 |
| 更换 Embedding 模型 | 必须强制重建 | 从 BGE-M3 切换到另一模型，即使文档 checksum 不变，也必须重新生成全部向量 |
| 修改分块或解析策略 | 必须强制重建 | chunk 长度从 1200 调整为 800，必须重新解析、切片和向量化 |

索引重建任务使用 `force=True`，因此它会绕过“内容未变化”的普通去重判断。这是有意设计的，
用于模型升级、向量维度变化、解析器升级、父节点修复和分块策略调整。哈希只能证明内容是否变化，
不能证明两个 Embedding 模型的向量空间兼容；即使两个模型都是 1024 维，也不能混用旧向量和新查询向量。

因此，换模型时必须执行：

```text
确认模型输出维度与 pgvector 列一致
  → 创建受控 reindex job
  → 使用 force=True 全量重建
  → 验证 Recall@K、MRR、引用完整性和越权零容忍
  → 验收通过后再将新模型作为查询模型
```

如果只修改环境变量而不重建，可能出现“查询使用新模型、数据库仍是旧模型向量”的静默质量下降；
维度不一致时则可能直接在 Embedding 校验或 PostgreSQL `vector` 写入阶段失败。

### 10.1.2 管理员上传文档的完整流程

管理员上传知识文档不是直接向量化，而是先经过身份校验、安全扫描、审核和暂存，只有审核通过
并且后台索引事务成功后，才成为可检索的 `PUBLISHED` 知识版本：

```text
管理员上传 PDF / DOCX / XLSX / Markdown
  → 校验签名 AgentContext、管理员角色和组织范围
  → 校验文件非空、文件大小、source_uri、标题和文档类型
  → 计算原始文件 SHA-256
  → 结构扫描或 ClamAV 扫描
  → 预解析文件并生成质量/审核报告
  → 文件写入私有暂存区或对象存储
  → 创建 knowledge_ingestion_jobs，状态 PENDING_REVIEW
  → 管理员或具备资质的专业人员审核
  → 审核通过后状态变为 QUEUED
  → Worker 领取任务，状态变为 INDEXING
  → 重新读取不可变暂存文件并再次校验原始文件 SHA-256
  → 重新解析、清洗、页面路由和语义切片
  → 计算规范化文档 checksum
  → 检查同 source_uri 的当前已发布版本是否未变化
  → 未变化则 SKIPPED_UNCHANGED，结束且不调用 Embedding
  → 有变化则创建文档版本、父节点和 Chunk
  → 计算每个 Chunk 的 content_hash
  → 批量调用当前 Embedding 模型并校验向量维度
  → 在同一事务中归档旧版本、写入文档/父节点/Chunk/向量
  → 事务提交后文档变为 PUBLISHED，任务变为 SUCCEEDED
```
...
| 阶段 | 持久化内容 | 示例 |
| --- | --- | --- |
| 原始文件身份 | `knowledge_ingestion_jobs.content_sha256` | `warmup.pdf` 的原始字节哈希为 `7e3a...91bc` |
| 审核绑定 | 审核报告绑定文件哈希、解析管线版本和审核策略版本 | 审核的是哈希 `7e3a...91bc` 的文件，执行时哈希变化则拒绝发布 |
| 暂存任务 | `knowledge_ingestion_jobs` | 文件上传后先是 `PENDING_REVIEW`，不参与 RAG 检索 |
| 文档身份 | `source_uri + version` 生成稳定文档 ID | `knowledge://fitness/warmup` 的第 2 版生成新的不透明 ID |
| 文档内容身份 | `knowledge_documents.checksum` | 清洗后正文不变时 checksum 不变，可跳过重复索引 |
| 父节点 | `knowledge_parents` | “热身与拉伸”整章作为命中 Chunk 的补充上下文，不单独生成向量 |
| Chunk 身份 | `knowledge_chunks.content_hash` 和稳定 Chunk ID | “热身 5 分钟”改成“热身 8 分钟”后，该 Chunk 哈希变化 |
| 向量 | `knowledge_chunks.embedding` | 每个 Chunk 由当前 Embedding 模型生成 1024 或 1536 维向量，必须与数据库列一致 |
| 最终发布 | 文档、父节点和 Chunk 在一个事务中提交 | 新版本写入失败时旧的 `PUBLISHED` 版本继续可检索 |

上传任务和文档版本是两个不同的概念。重复上传仍可能产生新的上传任务和审核报告，但只要内容去重
命中，就不会在知识文档表中产生新的检索版本，也不会重复消耗 Embedding 额度。

### 10.1.3 上传去重和异常分支示例

当前以同一 `source_uri` 的当前已发布文档和规范化内容 checksum 为主要去重依据：

| 场景 | 当前结果 | 具体例子 |
| --- | --- | --- |
| 同来源、同内容 | `SKIPPED_UNCHANGED`，不重新向量化 | `knowledge://fitness/warmup` 再次上传相同指南 |
| 文件名变化、来源和内容不变 | 仍然跳过 | `warmup-v1.pdf` 改名为 `warmup-final.pdf` 后再次上传 |
| 同来源、正文变化 | 创建新版本并重新向量化 | “热身 5 分钟”改成“热身 8 分钟”，版本 1 归档，版本 2 发布 |
| 内容相同、source_uri 不同 | 当前不会全库去重 | 同一 PDF 上传到 `knowledge://org/a` 和 `knowledge://org/b`，会被视为两个来源 |
| 内容相同、权限范围不同 | 不应简单合并 | 全平台指南和机构内部指南正文相同，但可见范围不同 |
| 审核被拒绝 | 保留任务和审核证据，不生成向量 | 医疗内容缺少专业审核决定，任务为 `REJECTED` |
| PDF 需要 OCR 但 OCR 不可用 | 阻断，不生成向量 | 图片密集页没有正文，当前本地 OCR 关闭，任务不能发布 |
| 审核后文件被替换 | Worker 哈希校验失败 | 审核报告绑定 A 文件，执行时暂存对象变成 B 文件 |
| Embedding 服务不可用 | 任务为 `FAILED`，旧版本保留 | 新版索引调用模型超时，线上仍使用旧版知识 |
| Embedding 维度错误 | 任务为 `FAILED` | 模型返回 1536 维，但数据库当前向量列为 1024 维 |
| Worker 中途崩溃 | 保留任务状态并允许有限重试 | Worker 写入前退出，重试时重新读取暂存文件 |
| 内容不变但只改权限 | 当前 checksum 去重可能跳过元数据更新 | 同一正文从 `GLOBAL` 改为 `ORGANIZATION`，需要显式强制重建或补元数据更新逻辑 |
| 更换 Embedding 模型 | 必须通过 `force=True` 重建 | 文档 checksum 不变，但从 BGE-M3 v1 切换到 v2，仍需重新生成所有向量 |
| 修改解析器或分块策略 | 必须强制重建 | Chunk 长度从 1200 改为 800，需重新解析、切片和向量化 |

当前正常上传的去重判断只解决“内容是否变化”，不能证明“向量模型是否兼容”。因此换模型时不能
依赖普通上传的 checksum 跳过逻辑，而必须创建索引重建任务。后续企业级增强应将：

```text
content_checksum       正文内容指纹
publication_fingerprint 正文 + 权限 + 生效时间 + 文档属性指纹
embedding_index_version 模型和向量索引版本
```

分开记录。这样可以做到：正文不变但权限变化时只更新 ACL 元数据；模型变化时强制重建向量；
解析策略变化时重新生成父子节点，并且每条检索结果都可以追溯到对应的解析器、模型和知识库版本。

### 10.1.4 为什么采用同来源去重，而不是直接全库合并

当前采用“同一 `source_uri` + 内容 checksum”的逻辑去重，而不是按全库内容指纹自动合并，
这是安全、权限、来源和版本优先的设计，不是遗漏了全库哈希。

#### 采用同来源去重的原因

1. **相同正文不代表相同权限**。

   例：全平台公开的训练指南和某教练的私有训练方案正文相同，但前者是 `GLOBAL`，后者是
   `PRIVATE`。如果直接合并向量，必须额外维护多套 ACL 关系，否则可能把私有知识错误召回给其他用户。

2. **相同正文不代表相同来源**。

   例：官方训练指南和机构内部改编版可能有相同段落，但引用时仍必须区分来源、版权责任主体、
   文档地址、页码和版本，不能只返回一个没有来源边界的 Chunk。

3. **文档生命周期可能不同**。

   例：机构 A 删除资料，而机构 B 仍然在使用相同内容。若两者直接共享一条物理记录，删除 A 时
   可能误删 B 的知识；独立文档记录则可以分别归档和审计。

4. **文档版本不能互相影响**。

   例：两个机构现在使用相同版本，但其中一个机构下个月发布了修订版，修订不能自动改变另一个
   机构仍在使用的旧版本。当前使用 `source_uri + version` 生成文档身份，版本边界清晰。

5. **哈希只能判断精确内容，不等于语义相同**。

   例：“训练前需要充分热身”和“运动前应进行动态热身”意思接近，但文字不同；相反，页码、
   标题路径、表格坐标或父节点不同，也可能使规范化内容指纹不同。因此不能把全库哈希当成语义去重。

当前 `knowledge_chunks` 直接绑定 `document_id`、组织、所有者、可见范围和角色，且检索时先执行
SQL ACL 过滤。保留不同来源的独立记录，符合当前表结构和企业安全优先原则；代价是不同来源之间可能
存在少量重复向量。

#### 企业级通常拆成逻辑去重和物理去重

```text
逻辑层：每个来源独立保留文档、版本、权限和引用关系
  → 保证安全、审计、生命周期和引用正确

物理层：相同文件或相同 Chunk 可以共享底层对象/向量
  → 需要独立的引用表和 ACL 关系，不能直接合并当前 knowledge_chunks
```

例如，同一个原始文件被两个机构上传，可以只保存一份对象存储副本：

```text
机构 A 文档 → object_hash = X
机构 B 文档 → object_hash = X
```

但仍然保留两条独立文档记录和各自权限。只有在新增 `document_chunk_refs`、共享 Chunk 内容表和
独立 ACL 关系后，才适合安全地共享底层向量。

#### 推荐的后续增强

1. **全库精确指纹用于重复提醒，不自动合并**。

   例：不同 `source_uri` 的规范化内容 hash 相同，后台提示“发现可能重复文档”，由管理员决定
   是否继续发布。

2. **原始文件 hash 用于对象存储物理去重**。

   例：两个上传任务引用同一个文件 SHA-256，只保留一份对象，但保留两条任务、审核和文档审计记录。

3. **语义相似检测只用于人工审核提示**。

   例：新文档与已有文档相似度很高，系统标记为“可能重复或内容冲突”，但不能自动删除，因为
   两份资料可能适用人群、安全限制或机构政策不同。

因此，当前“同来源去重”是正确性优先的基线；全库内容指纹可以作为重复提醒和存储优化能力补充，
但不能直接把不同来源的知识合并为一条可检索记录。
