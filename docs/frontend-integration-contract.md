...
本文是当前后端已经实现能力的前端交付说明。它只覆盖健身业务，不包含赛事、作品、活动运营、短信、Push、人工转接、图片动作框选和复杂训练扩展。

## 1. 当前交付边界

当前仓库没有小程序、管理端或教练端前端源码，因此本轮不凭空创建一套前端页面。后端已经提供稳定的 Agent API、确认 API、Memory API、通知 API、知识审核 API 和管理员运维 API；真正的前端工程应按本文接入。
...
8. 断网后恢复页面，使用原确认 ID 和请求 ID刷新，不新建业务动作。

这份契约完成的是后端接口到前端的交付边界，不代表独立前端工程已经开发完成。前端工程建立后，应把实际页面截图、联调请求和上述验收结果补充到交付报告中。
