...
当前 `fitness-web` 已完成能力目录、Agent 对话、业务工作台快捷入口、数据中心、Memory 收件箱/纠正/撤销、通知读取/已读/偏好、确认卡片和统一错误处理；管理员还可使用知识资料上传、任务详情、质量报告、批准/拒绝/重试和通知投递摘要。训练计划、预约、客服仍保持“对话查询 + 确认卡片”交互，不伪造尚未稳定的结构化业务 Schema；正式认证/BFF、预发布和浏览器端到端验收仍属于部署阶段工作。
