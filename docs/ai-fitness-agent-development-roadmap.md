...
- 已增加候选提取第一切片：仅对“请记住、我通常、我喜欢、我只有”等长期表达触发 DeepSeek JSON 提取；候选经过
  类型白名单、键名、长度、重复和健康敏感内容过滤，只作为当前回合的未确认上下文提供给 Supervisor。候选不会
  写入 `agent_memories`，若主模型决定调用保存工具，仍必须走 `interrupt()` 确认卡。

尚未完成且必须在后续阶段补齐：候选提取失败的独立指标、候选跨请求持久化和页面重新打开、用户逐条确认页面、
短期会话摘要、定时过期 Worker、Memory 脱敏保留策略和带脱敏样本的离线评测。当前版本仍采用“候选只读、显式
确认保存”作为安全基线，不从每轮聊天自动猜测并写入长期 Memory。
...
- Memory 第一切片：新增 `agent_memories` 迁移、结构化 Memory Service、主体/机构隔离、过期过滤、版本化覆盖和
  ...
  幂等请求 ID；训练计划生成只读取已确认且未过期的 Memory。新增只读候选提取服务，但候选跨请求持久化和
  短期会话 Memory 尚未完成。
...
4. **训练计划结构化生成与 RAG/Memory 组合（生成与确认创建联调、Memory 受控读取已完成）**：已在已验证
   ...
   程序校验，失败最多有限修复一次；集成测试已验证预览进入 `create_draft` 时先触发确认、确认前不写库。
   当前 Memory v1 支持显式确认后的结构化读取、覆盖和撤销，并已接入只读候选提取；任何训练计划结果仍必须
   进入教练审核，不能直接发布。下一步补齐候选跨请求持久化、用户候选管理和到期清理 Worker。
5. **多角色按页审核决策**：在不可变审核报告之上增加审核决定、身份/资质快照、
   页码/区域范围和发布凭证；教练只解除动作与一般训练安全领域，临床领域必须等待已验证专业资质，
   任何单个全局布尔值都不能解除页面门禁。
6. **原生 PDF 复杂版式与解析评测**：继续检测多栏顺序、图文关系、区域引用、跨页表格和断词；
   未达阈值不得发布知识版本，不把“少量文字的图片页”直接当作合格正文。
7. **知识库重建与前后对比复测**：保留原始文件，重新解析合格资料并生成父节点、子节点和
   Embedding；与 `docs/rag-baseline-20260812.md` 的 Recall@K、MRR 和引用结果对比。美国指南 PDF
   只有通过评测才能恢复索引。
8. **知识库生产闭环**：接入真实对象存储生命周期，将索引 Worker 部署为独立进程，完善失败重试、
   死信与审计；在 Linux CPU/GPU 节点联调 PaddleOCR，验证页码、表头、置信度和吞吐。
9. **Booking Agent**：实现预约、改约、取消预约，复用阶段 2 的 interrupt/确认凭证协议，并完成幂等、
   事务、审计和管理员/教练/
   学员权限测试。
10. **后续 Agent 能力**：依次推进 Operations Agent 与受控 Text-to-SQL、Memory、主动提醒、客服与
