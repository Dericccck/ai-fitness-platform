...
- 已增加正式 Memory 管理闭环：`0019` 新增不可变 `agent_memory_events`，保存、撤销和自动过期与正式 Memory
  ...
  `GET /api/v1/agent/memories`、`POST /api/v1/agent/memories/{memory_id}/revocations` 和 `/events` 接口，
  管理接口从签名主体反查机构并只允许本人访问。页面撤销本身就是显式确认，对话式撤销仍使用 `interrupt()`。

尚未完成且必须在后续阶段补齐：候选审批页面/通知、Memory 脱敏保留策略、正式 Memory 的纠正页面、短期会话摘要和
带脱敏样本的离线评测。
...
4. **训练计划结构化生成与 RAG/Memory 组合（生成与确认创建联调、Memory 受控读取已完成）**：已在已验证
   ...
   下一步补齐页面交互、通知、正式 Memory 纠正页面和 Memory 脱敏评测。
