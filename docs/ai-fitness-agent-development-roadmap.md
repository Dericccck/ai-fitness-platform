...
当前状态（2026-08-13）：已开始第一切片。复杂 PDF 解析和 OCR 不阻塞训练业务建模，但目录过滤、
...
当前未完成：通过 `fitness-core-gateway` 暴露训练写工具、训练执行记录、组次反馈、测量和阶段
调整，以及 Python Agent 的结构化计划生成器。这些不能用 Prompt 或内存对象替代。
...
1. **训练领域第一切片（当前进行中）**：通过 `fitness-core-gateway` 增加训练计划查询、提交审核、
   审核和发布写工具；所有写工具补确认凭证、幂等、资源级权限和 Agent 契约测试。
2. **训练执行闭环**：增加学员执行记录、组次、重量、次数、RPE、反馈、测量和教练阶段调整。
3. **多角色按页审核决策**：在不可变审核报告之上增加审核决定、身份/资质快照、
   页码/区域范围和发布凭证；教练只解除动作与一般训练安全领域，临床领域必须等待已验证专业资质，
   任何单个全局布尔值都不能解除页面门禁。
4. **原生 PDF 复杂版式与解析评测**：继续检测多栏顺序、图文关系、区域引用、跨页表格和断词；
   未达阈值不得发布知识版本，不把“少量文字的图片页”直接当作合格正文。
5. **知识库重建与前后对比复测**：保留原始文件，重新解析合格资料并生成父节点、子节点和
   Embedding；与 `docs/rag-baseline-20260812.md` 的 Recall@K、MRR 和引用结果对比。美国指南 PDF
   只有通过评测才能恢复索引。
6. **知识库生产闭环**：接入真实对象存储生命周期，将索引 Worker 部署为独立进程，完善失败重试、
   死信与审计；在 Linux CPU/GPU 节点联调 PaddleOCR，验证页码、表头、置信度和吞吐。
7. **Booking Agent**：实现预约、改约、取消预约，以及确认凭证、幂等、事务、审计和管理员/教练/
   学员权限测试。
