...
尚未完成且必须在后续阶段补齐：短期会话摘要、候选记忆自动提取、候选去重和用户逐条确认、定时过期 Worker、
前端查看/纠正/删除页面、Memory 脱敏保留策略和带脱敏样本的离线评测。当前版本采用“显式结构化保存”作为安全
基线，不从每轮聊天自动猜测并写入长期 Memory。
...
- Memory 第一切片：新增 `agent_memories` 迁移、结构化 Memory Service、主体/机构隔离、过期过滤、版本化覆盖和
  ...
  幂等请求 ID；训练计划生成只读取已确认且未过期的 Memory。自动候选提取和短期会话 Memory 尚未完成。
...
4. **训练计划结构化生成与 RAG/Memory 组合（生成与确认创建联调、Memory 受控读取已完成）**：已在已验证
   ...
   当前 Memory v1 只支持显式确认后的结构化读取、覆盖和撤销；任何结果仍必须进入教练审核，不能直接发布。
   下一步补齐候选提取、短期会话摘要和到期清理 Worker。
