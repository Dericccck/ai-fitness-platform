...
当前状态（2026-08-15）：预约可用性只读预检和创建预约第一条写链路已完成。创建预约已接入独立
`fitness-booking-service`，支持确认凭证、请求幂等、教练日期并发锁、合同课时事务扣减、审计和 Outbox；
改约、取消及营业时间/排班细粒度查询尚未开放；真实 MySQL 集成测试入口已建立，待连接专用测试库执行。
...
不接入短信（包括模拟短信）、Push，也不为了站内通知强行引入 RabbitMQ；RabbitMQ 只在后续主动提醒或跨服务事件
链路确有异步解耦需求时再建设。当前仍需补齐实际候选审批页面、外部通知渠道适配，以及带人工标注的短期摘要
...
当前 Memory 业务不接入短信（包括模拟短信）、Push 或 RabbitMQ；这些能力只有在主动提醒、跨服务事件或明确的外部
通知需求出现后，才进入单独建设阶段，并继续补齐 Memory 脱敏评测。
