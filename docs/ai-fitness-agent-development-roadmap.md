...
| 高频可撤销记录 | 学员逐组训练记录、草稿自动保存 | 采用页面显式提交或短会话范围授权，最终提交时确认；仍需幂等和审计 |
...
3. **规范化动作与摘要**：为每个写工具注册资源解析器、风险级别和确定性展示模板；使用稳定 JSON
   ...
   不能参与授权范围计算。
4. **LangGraph 等待节点**：在模型提出写工具后先校验参数、创建或复用确认单，再进入 `interrupt()`；
   只读工具继续直接执行。当前项目使用的 LangGraph 0.6.x 已具备 `interrupt/Command`，先在现有锁定
   版本上实现并增加兼容测试，不在同一提交中同时升级框架主版本。
5. **确认与恢复 API**：增加确认详情查询、批准、拒绝和幂等恢复；每次用最新 AgentContext 重新认证，
...
当前未完成：训练执行记录、组次反馈、测量和阶段调整，以及基于 RAG 和学员档案的 Python
结构化计划生成流程。这些不能用 Prompt 或内存对象替代。
...
2. **通用写操作 Human-in-the-loop 闭环（当前进行中）**：按 7.4 节先剥离 Checkpoint 中的签名上下文，
   在 Agent PostgreSQL 建立持久化确认单和确定性签发边界，再接入 LangGraph `interrupt()`、确认/拒绝与
   `Command(resume=...)` 恢复 API、断线/重启恢复和安全测试。第一批覆盖训练计划四个写工具，完成后
   供 Booking、Memory 等复用；Java Gateway 继续负责独立验签和最终业务权限复核。
3. **训练执行闭环**：增加学员执行记录、组次、重量、次数、RPE、反馈、测量和教练阶段调整；高频记录
   按风险分级采用页面提交或会话范围授权，不机械地对每组记录弹窗，但最终提交保持幂等和审计。
