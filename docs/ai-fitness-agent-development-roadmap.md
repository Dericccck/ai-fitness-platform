...
4. **训练计划结构化生成与 RAG/Memory 组合（第一切片已完成，Memory 尚未接入）**：已在已验证身份和
   已发布知识引用基础上生成结构化草案预览；模型输出、字段数量、训练日连续性和动作顺序均经过程序校验，
   失败最多有限修复一次。下一步是把用户确认后的草案 Payload 接入创建草案体验测试，再设计最小 Memory
   读取边界；任何结果仍必须进入教练审核，不能直接发布。
