...
尚未完成且必须在后续阶段补齐：候选审批页面/通知、Memory 脱敏保留策略、短期会话摘要和带脱敏样本的离线评测。
...
- Memory 第一切片：新增 `agent_memories` 迁移、结构化 Memory Service、主体/机构隔离、过期过滤、版本化覆盖和
  ...
  跨请求待确认查询、批准/拒绝 API、生命周期审计和候选到 ACTIVE Memory 的幂等晋级；短期会话 Memory 和候选
  前端交互尚未完成，到期 Worker 已独立部署入口。
...
4. **训练计划结构化生成与 RAG/Memory 组合（生成与确认创建联调、Memory 受控读取已完成）**：已在已验证
   ...
   计划结果仍必须进入教练审核，不能直接发布。候选到期清理 Worker、生命周期审计和基础指标已完成，下一步补齐
   页面交互、通知和 Memory 脱敏评测。
