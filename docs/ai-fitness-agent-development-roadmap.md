...
安排：当前知识源治理完成后、复杂 PDF 重新索引前、Booking Agent 开发前完成。该专项不与业务 Agent
开发混在同一个提交中，避免解析缺陷继续污染线上知识版本。
...
1. 完成复杂 PDF 参考资料治理，保留原始文件但移除不合格 PDF 的 RAG 派生索引。
2. 开发并验证阶段 4.1 PDF 解析质量专项，通过评测后再决定是否重新索引美国指南 PDF。
3. 接入真实对象存储生命周期，并将 Worker 部署为独立进程。
4. 在 Linux CPU/GPU 节点启动 PaddleOCR OCR 服务，使用真实扫描 PDF/表格评测集验证页码映射、
   表头保留、OCR 置信度和吞吐，再把结果接入持续质量门禁。
5. 在 RAG 骨架上接入预约写工具、确认凭证、幂等和审计。
6. 建立训练领域表、训练 Tool Gateway 和 Fitness Agent 的结构化计划流程。
7. 再推进 Operations Agent、Memory 和主动提醒等后续阶段。
