...
- 已增加候选管理 API：用户只能查询本人机构范围内的待确认候选，并通过决定请求 ID 执行批准或拒绝；批准先按候选
  稳定 ID 幂等创建 `ACTIVE` Memory，再把候选置为 `APPROVED`，拒绝不会创建 Memory。候选审批不是自动授权，也
  不绕过现有 `fitness.memory.save.v1` 的确认边界。

尚未完成且必须在后续阶段补齐：候选提取失败的独立指标、候选审批页面/通知、定时过期 Worker、Memory 脱敏保留
策略、短期会话摘要和带脱敏样本的离线评测。当前版本仍采用“候选不等于事实、只有显式批准才保存”作为安全
基线，不从每轮聊天自动猜测并写入正式长期 Memory。
...
- Memory 第一切片：新增 `agent_memories` 迁移、结构化 Memory Service、主体/机构隔离、过期过滤、版本化覆盖和
  ...
  跨请求待确认查询、批准/拒绝 API 和候选到 ACTIVE Memory 的幂等晋级；短期会话 Memory、到期 Worker 和候选
  前端交互尚未完成。
...
4. **训练计划结构化生成与 RAG/Memory 组合（生成与确认创建联调、Memory 受控读取已完成）**：已在已验证
   ...
   计划结果仍必须进入教练审核，不能直接发布。下一步补齐候选到期清理 Worker、页面交互和 Memory 脱敏评测。
