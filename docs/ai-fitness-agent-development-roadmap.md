...
- Operations Agent 第一切片：Java Gateway 增加管理员专用的固定经营指标目录，支持预约总量、预约状态分布、课程预约量、
  ...
  后续继续建设更复杂指标的时间桶、同比、自然语言 Text-to-SQL 的正式指标映射和趋势解释。
...
6. **Operations Agent 与受控 Text-to-SQL（进行中）**：已完成固定指标目录、管理员权限边界、常见中文问题的指标/日期提示、
   ...
   当前周期和环比上一周期已分别持久化到 PostgreSQL `agent_operations_query_audits`，只保存主体/角色快照、机构、固定指标、
   ...
   一致；指标漂移、日期扩大、同比和组织越权在 Java Gateway 前 fail-closed。当前同比仍保持澄清策略，不自动猜测统计口径。
   已集中维护固定指标目录，报表返回指标的业务口径、维度含义、支持时间桶和环比能力；歧义经营问题会列出可选指标并要求
   明确口径。新增指标必须同步补充 Gateway 固定 SQL、权限、审计和评测。
   后续继续建设更多指标的受控时间桶、正式同比口径评测、
