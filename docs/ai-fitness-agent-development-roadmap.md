...
Booking 和 Fitness 的真实 dry-run 已完成，当前唯一进行中的步骤切换为“真实业务闭环的角色验收”：继续使用受控本地
测试数据，分别以管理员、教练和学员身份验证训练计划的草案生成、教练审核、发布可见性和学员只能执行已发布计划的边界；
真实写入仍需每个场景单独明确授权，不能把 dry-run 的通过当成已完成写入验收。
