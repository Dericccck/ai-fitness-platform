...
当前状态（2026-08-15）：预约可用性只读预检、创建预约和改约第一批写链路已完成。创建预约与改约已接入独立
...
改约 v1 只调整教练和时间，不更换合同/课程、不重复扣课时，并通过 `expectedStartTime` 防止确认后的并发覆盖；取消及营业时间/排班细粒度查询尚未开放。
真实 MySQL 集成测试入口已建立，待连接专用测试库执行。

工作内容：

- 接入阶段 2 的预约工具和独立预约写服务。
- 处理无合同、合同冻结、课时不足、教练不属于组织、请假、非营业日和时间冲突。
- 实现预约、改约和取消的多轮确认；改约先限定为调整教练/时间，跨合同换课留到后续业务评审。
- 建立并发预约、重复请求和失败补偿测试。

完成标准：Booking Agent 与现有 Java 预约规则一致；预约创建和改约已具备确认、幂等、审计及并发保护，预约取消仍需完成后才算本阶段闭环；并发场景不产生重复预约或错误覆盖。
...
不接入短信（包括模拟短信）和 Push，也不为了站内通知强行引入 RabbitMQ；RabbitMQ 已在 Booking Agent
预约跨服务事件中单独建设。当前仍需补齐实际候选审批页面、外部通知渠道适配，以及带人工标注的短期摘要
...
5. **Booking Agent**：实现预约、改约、取消预约，复用阶段 2 的 interrupt/确认凭证协议，并完成幂等、
   事务、审计和管理员/教练/学员权限测试。
6. **Operations Agent 与受控 Text-to-SQL**：在真实业务事实库只读视图、组织范围和资源限制基础上建设
   经营查询能力，继续忽略赛事、作品和活动运营遗留模块。
7. **Memory 与主动提醒**：继续完善候选审批、摘要质量、通知模板和主动提醒业务；短信、Push、RabbitMQ
   只在真实业务闭环需要时单独接入，不为了占位提前扩展。
