...
2. **发布前 fail-closed 门禁（本轮）**：路由证据进入统一 `ParsedDocument` 和离线质量报告；待 OCR、
   ...
   普通业务接口和 LLM 无权覆盖。
3. **审核报告持久化与 API（下一切片）**：上传后、批准前保存解析器版本、页面画像、阻断原因和建议
   审核角色；管理员查看通用质量结果，动作/姿态/禁忌交由教练，疾病和高风险运动建议必须由具备专业
   资格的人员审核。审核决定按文档版本和页码留痕，不能用一个全局布尔值绕过门禁。
4. **原生 PDF 复杂版式专项**：在 macOS 可完成多栏顺序、页眉页脚、目录、跨页表格、断词、图注关联
   和区域引用优化，并使用真实健身资料建立解析黄金集与误报/漏报指标。
5. **Linux OCR 联调（环境具备后）**：接入 PaddleOCR，验证组合路由状态只解除 OCR 部分；评测中文、
   数值、单位、表格、页码、吞吐和资源占用。OCR 无法判断动作标准性，也不能替代教练或医疗审核。
6. **审核通过后重建知识库**：只有页面路由、质量阈值、权限继承和审核凭证全部满足时，才重建父子
...
1. **解析与检索自动化质量门禁（当前进行中）**：检测噪声率、短碎片率、重复率、父节点完整性、
   表格完整性、缺失页、图片密集度、多栏顺序、图文关系和引用页码准确性；未达阈值不得发布知识版本。
   质量门禁完成后再进行 OCR/视觉内容路由，不把“少量文字的图片页”直接当作合格正文。
