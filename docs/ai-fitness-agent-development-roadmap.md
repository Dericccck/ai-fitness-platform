...
> 2026-09-05 当前优化入口：[Agent 与 RAG 版本治理优化计划](agent-rag-versioning-optimization-plan.md)。
> 根据文档上传/哈希/去重、版本归档、SQL 权限过滤、模型切换、观测评测和回滚讨论统一规划。
> 固定顺序：发布正确性 → 发布指纹与观测 → 独立索引构建与切换 → 四领域基线 → 性能优化 → 按需历史治理。
> 第 1 批已完成代码实现和本地回归：来源作用域、元数据指纹更新、上传幂等/并发约束；真实 PostgreSQL 集成验收与后续索引构建仍按专项计划推进。
...
本轮继续完成前端确认状态闭环：确认卡片首次展示后会读取脱敏确认单详情；批准后在非终态期间自动每 2 秒刷新，
也提供手动刷新入口，避免把 `APPROVED` 误显示为业务已经成功。批准/拒绝两类动作在同一确认卡片生命周期内分别固定
`decision_request_id`，网络失败后重试会复用原决定请求 ID，符合后端幂等契约。`make web-check` 已再次通过。

本轮新增前端“业务工作台”视图：根据服务端能力目录动态展示业务资料、预约、训练计划、个人偏好、经营分析和客服等领域入口，
每个快捷入口只负责带入预置查询意图并回到同一 Agent 会话，实际权限、数据范围和工具选择仍由服务端确定；没有能力的领域不会显示，
避免前端按钮与角色权限脱节。当前仍未把查询结果伪装成固定表格，待后端提供稳定领域响应 Schema 后再增加训练计划、预约、客服工单和经营指标的结构化卡片。

本轮继续完成前端数据中心：已接入 Memory 待确认收件箱、已确认 Memory、站内通知、管理员经营指标目录/查询审计、知识任务和索引重建任务。
Memory 候选批准/拒绝使用按候选固定的决定请求 ID，通知标记已读保持幂等；管理员数据仅在签名角色包含管理员权限时请求，普通角色不显示管理员数据卡片。
机构范围从本地开发配置读取，生产环境仍必须由认证服务/BFF 提供，前端不解析或修改 AgentContext。

本轮前端收口补齐现有后端契约：数据中心支持正式 Memory 的版本化纠正和撤销、通知偏好开关；管理员治理区支持知识资料 multipart 上传、任务详情、解析质量报告、
安全扫描摘要、批准/拒绝/失败重试，并展示 IN_APP 通知投递摘要。索引重建看板已改用后端真实字段
