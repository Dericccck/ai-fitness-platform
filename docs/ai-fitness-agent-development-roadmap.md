...
Booking 和 Fitness 的真实 dry-run、Agent 层角色矩阵、训练服务最终权限修复以及学员越权真实拒绝验收已完成，当前唯一进行中的步骤切换为“Gateway/训练服务的完整真实角色验收”：
继续使用受控本地测试数据，分别以管理员、教练和学员身份验证计划资源范围、草案生成、教练审核、发布可见性和学员只能执行已发布计划
的边界；真实写入仍需每个场景单独明确授权，不能把 dry-run 或 Agent 层单测的通过当成已完成写入验收。
