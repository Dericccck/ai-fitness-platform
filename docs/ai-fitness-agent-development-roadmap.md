...
安排：先完成当前 RAG 的真实检索基线验证，再开始本专项；本专项必须在复杂 PDF 重新索引前和
Booking Agent 开发前完成。该专项不与业务 Agent 开发混在同一个提交中，避免解析缺陷继续污染
线上知识版本。

进入条件：

- 使用真实 PostgreSQL、pgvector、本地 BGE-M3 和 BGE-Reranker 跑通 RAG 检索接口。
- 使用“高温天气怎么运动”“如何提高肌肉耐力”等健身问题记录优化前基线。
- 基线必须记录候选召回、Reranker 排序、父节点展开、引用来源/页码和权限过滤结果。
- 对网页导航、短碎片、表格拆分、断词等问题保留可复现样本，作为专项验收前后的对照数据。
...
1. **真实 RAG 基线验证（当前唯一进行中的下一步）**：调用真实检索接口，验证混合召回、
   Reranker、父节点展开、引用来源/页码和权限过滤；记录 Recall@K、MRR、越权结果和脏数据样本。
2. **复杂 PDF 解析质量专项**：过滤网页模板、目录和页眉页脚，修复异常空格/断词，恢复标题、
   列表、表格及跨页表头结构；清洗后再生成父子节点，不合格文档转人工审核或 `REFERENCE_ONLY`。
3. **解析与检索自动化质量门禁**：检测噪声率、短碎片率、重复率、父节点完整性、表格完整性、
   引用页码准确性、Recall@K、MRR 和越权召回，未达阈值不得发布知识版本。
4. **知识库重建与前后对比复测**：保留原始文件，重新解析合格资料并生成父节点、子节点和
   Embedding；与步骤 1 的基线对比。美国指南 PDF 只有通过评测才能恢复索引。
5. **知识库生产闭环**：接入真实对象存储生命周期，将索引 Worker 部署为独立进程，完善失败重试、
   死信与审计；在 Linux CPU/GPU 节点联调 PaddleOCR，验证页码、表头、置信度和吞吐。
6. **Booking Agent**：实现预约、改约、取消预约，以及确认凭证、幂等、事务、审计和管理员/教练/
   学员权限测试。
7. **训练领域与 Fitness Agent**：建立训练档案、动作库、结构化计划、教练审核、学员执行、训练记录、
   阶段评估和调整流程，并受控结合专业知识 RAG。
8. **后续 Agent 能力**：依次推进 Operations Agent 与受控 Text-to-SQL、Memory、主动提醒、客服与
