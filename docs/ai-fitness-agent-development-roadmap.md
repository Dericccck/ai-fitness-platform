...
- Operations Agent 第一切片：Java Gateway 增加管理员专用的固定经营指标目录，支持预约总量、预约状态分布、课程预约量、
  ...
  和非管理员拒绝测试已补齐。自然语言 Text-to-SQL 的指标映射、只读视图和审计仍未完成。
...
6. **Operations Agent 与受控 Text-to-SQL（进行中）**：已完成固定指标目录和管理员权限边界；继续建设真实业务事实库只读视图、
   组织范围和资源限制基础上的自然语言指标解析、SQL 生成前校验、查询审计和结果解释，继续忽略赛事、作品和活动运营遗留模块。
