"""执行 Booking Agent 真实确认流程和可选写入联调。

默认只验证自然语言请求能进入 Booking 路由并生成待确认单，不会创建预约。只有显式
...
def _arguments() -> tuple[argparse.Namespace, str, str, str]:
    ...
    add_common_arguments(parser, description="Booking Agent 真实联调")
    return (
        parser.parse_args(),
...
if __name__ == "__main__":
    raise SystemExit(run_main(_arguments, label="Booking"))
