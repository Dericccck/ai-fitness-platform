"""执行健身 Agent 训练计划草案真实确认流程和可选写入联调。

默认只验证训练计划创建请求能进入健身路由并生成待确认单，不会写入训练服务。
...
def _arguments() -> tuple[argparse.Namespace, str, str, str]:
    ...
    add_common_arguments(parser, description="健身 Agent 训练计划真实联调")
    return (
        parser.parse_args(),
...
if __name__ == "__main__":
    raise SystemExit(run_main(_arguments, label="健身"))
