...
def downgrade() -> None:
    """存在新客量审计记录时拒绝回退，避免静默删除审计事实。"""

    connection = op.get_bind()
    new_customer_count = connection.execute(
        sa.text(
            "SELECT COUNT(*) FROM agent_operations_query_audits WHERE metric = 'NEW_CUSTOMER_COUNT'"
        )
    ).scalar_one()
    if new_customer_count:
        raise RuntimeError("无法回退新客量指标迁移，因为仍存在新客量审计记录")
    op.drop_constraint(
        "ck_agent_operations_audit_metric",
        "agent_operations_query_audits",
        type_="check",
