...
help:
	...
	@echo "  agent-domain-subgraphs-session-live-check Run same-session cross-domain read-only smoke checks"
	...
	@echo "  agent-release-components-check Generate v2 component digests from real Prompt/Tool/Model inputs"
	@echo "  production-config-check Validate all production environment templates and safety switches"
	@echo "  production-runtime-config-check Validate injected production config files without printing secrets"
...
trulens-infra-up:
	docker compose -f $(COMPOSE_FILE) up -d agent-trulens-postgres
...
agent-domain-subgraphs-session-live-check: agent-operations-live-preflight
	cd $(AGENT_DIR) && UV_CACHE_DIR=$(UV_CACHE_DIR) uv run python scripts/domain_subgraphs_session_live_check.py
...
agent-release-components-check:
	cd $(AGENT_DIR) && UV_CACHE_DIR=$(UV_CACHE_DIR) uv run python scripts/build_release_components.py $(ARGS)

production-config-check:
	cd $(AGENT_DIR) && UV_CACHE_DIR=$(UV_CACHE_DIR) uv run python scripts/production_config_contract_check.py

production-runtime-config-check:
	@test -n "$(PRODUCTION_ENV_DIR)" || (echo "请先设置 PRODUCTION_ENV_DIR（仓库外部的五份生产配置目录）"; exit 1)
	cd $(AGENT_DIR) && UV_CACHE_DIR=$(UV_CACHE_DIR) uv run python scripts/production_runtime_config_check.py --env-dir "$(abspath $(PRODUCTION_ENV_DIR))"

agent-migration-check:
	cd $(AGENT_DIR) && UV_CACHE_DIR=$(UV_CACHE_DIR) uv run python scripts/migration_contract_check.py

agent-migration-live-check:
	cd $(AGENT_DIR) && UV_CACHE_DIR=$(UV_CACHE_DIR) uv run python scripts/migration_live_check.py $(ARGS)
